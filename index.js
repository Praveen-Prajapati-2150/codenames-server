...
// Define REST API routes with Express
app.get('/api', (req, res) => {
...
io.on('connection', (socket) => {
  ...
  socket.on(
    ...
    ({ roomId, cardId, nickName, updatedWords }) => {
      try {
        ...
        const existingWordList = initializeWordListState(roomId, updatedWords);
  ...
  socket.on('join-room', ({ roomId, team, type, nickName }) => {
    try {
      ...
      roomState.redTeam = roomState.redTeam.filter((name) => name !== nickName);
